...
제어판의 **기능키 · OSD → 화면 영역 캡처**에서 단축키를 고르고
**선택한 캡처 단축키 적용**, **원래 설정 복원**, **상태 확인**을 사용할 수 있습니다.
단축키를 누르면 영역을 드래그로 선택해 클립보드로 바로 복사합니다. 파일로 저장하지 않습니다.

Cinnamon 전용입니다. Cinnamon에는 xfconf가 없어 키 설정이 GSettings
`org.cinnamon.desktop.keybindings.media-keys`의 `area-screenshot-clip`에 있습니다.
XFCE·KDE·GNOME 세션에서는 지원하지 않는다고 표시하고 아무것도 바꾸지 않습니다.

고를 수 있는 단축키는 고정 목록입니다. 임의로 입력한 값은 GSettings에 쓰지 않습니다.

| 단축키 | 설명 |
| --- | --- |
| `Super + Shift + S` | Windows 방식 (기본 제안) |
| `Ctrl + Shift + PrtSc` | Cinnamon 기본값 |
| `Super + PrtSc` | |
| `Super + Shift + 4` | Mac 방식 |

적용 전에 `media-keys`와 `wm` 스키마를 훑어 같은 키를 쓰는 다른 기능이 있으면
그 이름을 알려주고 거부합니다. 처음 적용할 때의 원래 값을 백업하며, 다시 적용해도
최초 원래 값을 유지합니다. 복원은 이 앱이 설정한 값이 그대로일 때만 되돌리고,
사용자가 이후 직접 바꿨으면 그 값을 보존합니다. 외부에서 값이 바뀌면 적용을 거부하고
상태에 표시합니다.

터미널에서는 `gf63-control-setup --capture-shortcut`으로 적용하고
`--restore-capture-shortcut`으로 되돌립니다. `gf63-control-setup --restore`에도 포함됩니다.

Cinnamon 기본 캡처 단축키는 그대로 둡니다. `PrtSc`는 전체 화면 저장,
`Shift + PrtSc`는 영역 저장, `Alt + PrtSc`는 활성 창 저장입니다.
